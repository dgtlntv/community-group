...
export function loadConfig(): ExamplesConfig {
  ...
  try {
    ...
    throw new Error(
      `Failed to load tests config at ${configPath}: ${message}`,
    );
  }
}
 ...
 * Returns the first matching type key, or `undefined`.
 */
 ...
 *
 * Returns stale entries whose fingerprints don't appear in the
 * current set of examples (likely due to edits or removals).
 */
 ...
 * detection-based classification. Returns `undefined` for
 * unclassified blocks.
 */
