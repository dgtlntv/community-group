...
- **`versions[].version`** — the DTCG specification version (e.g. `"2025.10"`). Determines which source directory (`src/2025.10/`) to read from and the output subdirectory (`dist/2025.10/`, `www/public/schemas/2025.10/`).
...
- **`outputDirs`** — paths relative to this package where bundled schemas are written. Each gets a `<version>/` subdirectory.

## Adding a new schema version

1. Create source schemas under `src/<version>/`.
2. Add an entry to the `versions` array in `schemas.config.json`.
3. Run the build.
