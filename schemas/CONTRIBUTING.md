...
- Keep `$ref` URIs consistent. Every `$ref` in a sub-schema must use the full `$id` URI of the target, including the correct version segment.
- Run the build before testing. Tests run against the bundled output in `dist/`, not the source files.
- One entry schema, one output file. Each object in `entrySchemas` produces exactly one bundled file with all internal `$ref`s resolved.
